...
import { CONDENSED_NODES, EXPANDED_NODES } from '@constants/graph-data'
interface HexagonExpandedProps {
  height?: number
  width?: number
}
...
const HexagonExpanded: React.FC<HexagonExpandedProps> = ({ width = 480, height = 480 }) => {
  ...
  const derivedWidth = isMobile && boxRef.current ? width - 150 : width
  const derivedHeight = isMobile && boxRef.current ? height - 150 : height
  ...
  useEffect(() => {
    if (boxRef.current) {
      cytoscapeRef.current = cytoscape({
        ...
        elements: EXPANDED_NODES.elements,
        style: [
          {
            selector: 'node',
            style: {
      ...
      cytoscapeRef.current.on('tap', onTapHandler)
      initializeTippy()
    }
  ...
  const onTapHandler = (event: cytoscape.EventObject) => {
    ...
    const toggleValue = toggleRef.current
    if (_private.data && _private.group === 'nodes' && cy) {
      const privateData = _private.data
      if (privateData.id === 'n1') {
        cy.elements().remove()
        if (toggleValue) {
          cy.add(EXPANDED_NODES.elements)
        } else {
          cy.add(CONDENSED_NODES.elements)
        }
...
export default HexagonExpanded
